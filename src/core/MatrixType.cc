...
using data_t = Value::MatrixType::data_t;

MatrixType::data_t buildMatrixData(size_t rows, size_t cols, bool dynamic) {
  if (!dynamic) {
    switch (rows) {
      case 2:
        switch (cols) {
...
MatrixType::MatrixType(size_t rows, size_t cols, bool dynamic, bool is_vector) : is_vector(is_vector) {
...
bool Value::MatrixType::resize(size_t rows) {
...
void Value::MatrixType::seal(size_t sealed_rows) {
...
VectorType Value::MatrixType::toVector(EvaluationSession *session, int size) const {
...
double Value::MatrixType::operator()(size_t row, size_t col) const {
...
{
  size_t index;
  bracket_visitor(size_t index) : index(index) {}
...
#define RETURN_ROW_AS_COL_VECTOR(type) \
        { \
          ...
          *v = mat->block(index, 0, 1, mat->cols()).transpose(); \
          return MatrixPtr(MatrixType(std::move(v), /* is_vector= */ true)); \
        }
...
Value MatrixType::operator[](size_t index) const
{
  return std::move(boost::apply_visitor(bracket_visitor(index), data));
}
...
Value MatrixType::operator[](const std::vector<size_t> &indices) const
{
  return std::move(boost::apply_visitor(multibracket_visitor(indices), data));
}

struct set_column_visitor : public boost::static_visitor<bool> 
{
  size_t index;
  bool element_is_vector;
  set_column_visitor(size_t index, bool element_is_vector) : index(index), element_is_vector(element_is_vector) {}
...
bool Value::MatrixType::set_row(size_t index, const MatrixType& element) {
  return boost::apply_visitor(set_column_visitor(index, element.is_vector), data, element.data);
}
...
bool Value::MatrixType::set_range(size_t index, const MatrixType& element) {
...
bool Value::MatrixType::set(size_t index, double value) {
...
// template <int N, int M, class T> struct matrix_traits<Value::MatrixType::LazyMatrix<N, M, T>> {
//   typedef std::integral_constant<int, N> rows;
//   typedef std::integral_constant<int, M> cols;
// };

template <class Op>
struct cwise_op_visitor : public boost::static_visitor<boost::optional<MatrixType>>
{
  const Op &op;
  bool is_vector;
  cwise_op_visitor(const Op &op, bool is_vector) : op(op), is_vector(is_vector) {}

  template <class A, class B>
  boost::optional<MatrixType> operator()(const A& op1, const B& op2) const {
  ...
  boost::optional<MatrixType> operator()(const std::shared_ptr<A>& op1, const std::shared_ptr<B>& op2) const {
    ...
    auto m = std::make_shared<Eigen::Matrix<double, matrix_traits<A>::rows::value, matrix_traits<A>::cols::value>>(op1->rows(), op1->cols());
    op(*op1, *op2, *m);
    // TODO(ochafik): resize when dynamic
    
    return MatrixType(std::move(m), is_vector);
  }
};
...
boost::optional<MatrixType> Value::MatrixType::operator+(const MatrixType& other) const {
  return boost::apply_visitor(cwise_op_visitor<plus_op>(plus_op(), is_vector || other.is_vector), data, other.data);
}
...
boost::optional<MatrixType> Value::MatrixType::operator-(const MatrixType& other) const {
  return boost::apply_visitor(cwise_op_visitor<minus_op>(minus_op(), is_vector || other.is_vector), data, other.data);
}
...
// boost::optional<MatrixType> Value::MatrixType::operator/(const MatrixType& other) const {
//   // return boost::apply_visitor(cwise_op_visitor<divide_op>(divide_op(), is_vector || other.is_vector), data, other.data);
//   // // assert(false);
//   return boost::none;
//   // return apply_visitor2(cwise_op_visitor<divide_op>(divide_op()), data, other.data);
// }

struct cross_visitor : public boost::static_visitor<boost::optional<MatrixType>> 
{
  template <class A, class B>
  boost::optional<MatrixType> operator()(const A& op1, const B& op2) const {
  ...
  template <int Rows1, int Cols1, int Rows2, int Cols2,
    std::enable_if_t<
      ...
      (Cols1 == Eigen::Dynamic || Cols1 == 1) &&
      (Cols2 == Eigen::Dynamic || Cols2 == 1), int> = 0>
  boost::optional<MatrixType> operator()(const std::shared_ptr<Eigen::Matrix<double, Rows1, Cols1>> &op1,
    ...
    return MatrixType(std::move(r), /* is_vector= */ true);
  }
};

boost::optional<MatrixType> Value::MatrixType::cross(const MatrixType &other) const {
...
#define SCALAR_VISITED_OP_IMPL(type, name) \
  ...
  type Value::MatrixType::name() const { \
...
// TODO(ochafik): allow is_vector on left of products!
template <bool lhs_is_vector>
struct product_visitor;

template <> struct product_visitor</* lhs_is_vector= */ false> : public boost::static_visitor<boost::optional<MatrixType>>
{
  bool rhs_is_vector;
  product_visitor(bool rhs_is_vector) : rhs_is_vector(rhs_is_vector) {}

  template <class A, class B>
  boost::optional<MatrixType> operator()(const A& out, const B& item) const {
  ...
  boost::optional<MatrixType> operator()(const std::shared_ptr<A>& op1, const std::shared_ptr<B>& op2) const {
    ...
    return MatrixType(std::move(m), rhs_is_vector);
  }
  ...
  boost::optional<MatrixType> operator()(const std::shared_ptr<A>& op1, const std::shared_ptr<B>& op2) const {
    ...
    // TODO: lazy matrix for types that make sense.
    auto m = std::make_shared<Eigen::MatrixXd>(op1->rows(), op2->cols());
    m->noalias() = *op1 * *op2;
...
template <> struct product_visitor</* lhs_is_vector= */ true> : public boost::static_visitor<boost::optional<MatrixType>>
{
  bool rhs_is_vector;
  product_visitor(bool rhs_is_vector) : rhs_is_vector(rhs_is_vector) {}
  ...
  template <class A, class B,
    typename std::enable_if_t<
        (matrix_traits<A>::rows::value == matrix_traits<B>::rows::value ||
        matrix_traits<A>::rows::value == Eigen::Dynamic ||
        matrix_traits<B>::rows::value == Eigen::Dynamic) &&
        std::is_assignable<
          MatrixType::data_t, 
          typename std::shared_ptr<Eigen::Matrix<double, matrix_traits<A>::cols::value, matrix_traits<B>::cols::value>>>::value,
      int> = 0>
  boost::optional<MatrixType> operator()(const std::shared_ptr<A>& op1, const std::shared_ptr<B>& op2) const {
    if (op1->rows() != op2->rows()) {
      return boost::none;
    ...
    // TODO: lazy matrix for types that make sense.
    auto m = std::make_shared<Eigen::Matrix<double, matrix_traits<A>::cols::value, matrix_traits<B>::cols::value>>(op1->cols(), op2->cols());
    m->noalias() = op1->transpose() * *op2;
  ...
  boost::optional<MatrixType> operator()(const std::shared_ptr<A>& op1, const std::shared_ptr<B>& op2) const {
    ...
    // TODO: lazy matrix for types that make sense.
    auto m = std::make_shared<Eigen::MatrixXd>(op1->cols(), op2->cols());
    m->noalias() = op1->transpose() * *op2;
...
bool Value::MatrixType::operator<(const MatrixType& other) const {
