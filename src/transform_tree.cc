...
AbstractNode *transform_tree(AbstractNode *node)
{
  ...
  for (auto it = node->children.begin(); it != node->children.end(); it++) {
    ...
    *it = simpler_child;
    ...
    LOG(message_group::None, Location::NONE, "",
      "[transform_tree] Ignoring tree with special tags");
#endif
		return node;
	}
  ...
  if (Feature::ExperimentalFlattenChildren) {
    if (list) {
      // Flatten lists.
...
#ifdef DEBUG
        LOG(message_group::None, Location::NONE, "",
          "[transform_tree] Found csg node with %1$d children", new_node->children.size());
#endif
