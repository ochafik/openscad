...
private:
  ...
  std::vector<std::shared_ptr<const Polygon2d>> collectPolygon2d(const std::shared_ptr<const Geometry>& geom);
  ...
  std::unique_ptr<Geometry> applyFill2D(const AbstractNode& node);
  ...
  std::unique_ptr<Geometry> applyToChildren2D(const AbstractNode& node, OpenSCADOperator op);
