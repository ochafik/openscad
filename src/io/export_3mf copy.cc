...
static bool append_polyset(const PolySet& ps, PLib3MFModelMeshObject *& model)
{
  ...
  if (ps.color.isValid()) {
    std::string materialName = "Color #...";
    MODELMESHCOLOR_SRGB color;
    // PLib3MFModelBaseMaterial
    lib3mf_floatrgbatocolor(ps.color[0], ps.color[1], ps.color[2], ps.color[3], &color);
    // auto color = model->FloatRGBAToColor(ps.color[0], ps.color[1], ps.color[2], ps.color[3]);

    // Lib3MF_BaseMaterialGroup pBaseMaterialGroup;
    PLib3MFModelBaseMaterial *pBaseMaterialGroup;
    lib3mf_model_addbasematerialgroup(model, &pBaseMaterialGroup);

    // Lib3MF_uint32 propertyId = 0;
    unsigned int propertyId = 0;
    lib3mf_basematerialgroup_addmaterial(pBaseMaterialGroup, materialName.c_str(), color, &propertyId);

    // Lib3MF_uint32 resourceId;
    unsigned int resourceId;
    lib3mf_resource_getresourceid(pBaseMaterialGroup, &resourceId); 

    lib3mf_meshobject_setobjectlevelproperty(mesh, resourceId, propertyId);

    // PBaseMaterialGroup pBaseMaterialGroup = model->AddBaseMaterialGroup();
    // Lib3MF_uint32 matID = pBaseMaterial->AddMaterial("Color #...", color);
    // meshObject->SetObjectLevelProperty(pBaseMaterialGroup->GetResourceID(), matID);
  }
